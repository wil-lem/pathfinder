...
class GameRound {
    ...
    cardAllowedOnTable(card) {
        ...
        for(var i in this.cards) {
            ...
            // If the check card is not on the table, skip it
    ...
    getPlayerPlayableCards(player) {
        var cards = this.getPlayerHand(player)
        return cards.filter( card => {
            return this.cardAllowedOnTable(card);
        });
    }
    ...
    passTurn(person) {
        ...
        var playableCards = this.getPlayerPlayableCards(person);

        if(playableCards.length > 0) {
            this.illegalPasses.push(person);
            // person.emitError('Cannot pass ' + playableCards.length);
            // return;
        }


        person.addAction({action:'passed'});
    ...
    cardToTable(card) {
        ...
        card.owner.addAction({action:'playcard',card:card.export()});
    ...
    showScores() {
        ...
        this.game.players.forEach(player => {
            ...
            var illegalPassCount = 0;
            if(this.illegalPasses.indexOf(player) > -1) {
                this.illegalPasses.forEach(cheater => {
                   illegalPassCount += (cheater == player) ? 1 : 0;
                });
            }
            ...
            players.push({
                ...
                illegalPassCount: illegalPassCount,
            });
            console.log(player.name,illegalPassCount);
